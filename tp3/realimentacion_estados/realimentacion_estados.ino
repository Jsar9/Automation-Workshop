...
float referencia [2] = {0,0}; // Referencia:  phi | theta.
float u = 0.0;
float u_servo = 0.0;
float z[2] = {0.0, 0.0};  // error
float K_i[2] ={0, 0};  // Constante de integración.
float K[4] = {200, 90, 12, 6}; // Ganancias de realimentación.
float M_ff [2] = {0,0}; // Matriz de Feedforward
...
void loop() {
  ...
  if (currentMillis - previousMillis_sensors >= INTERVAL_SENSORS) //&& (pot_angle == SERVO_MIN_ANGLE_ROUTINE || pot_angle == SERVO_MAX_ANGLE_ROUTINE) ) { 
    ...
    //------------------------------------------------------------------------------------------------------------------------- FIN SENSORES
      ...
      for (int i = 0; i < 4; i++) {
      x_hat[i] = constrain(x_hat[i], -1000.0, 1000.0);  // evitar explosión
      u -= K[i] * x_hat[i];
      }
      ...
      // Se calcula la salida estimada estimada: y_hat = C_d * x_hat
      ...
      // Calcular error: y - y_hat
      float error[2];
      for (int i = 0; i < 2; i++) {
      error[i] = y[i] - y_hat[i];
      }

      // Calcular x_hat_next = A_d * x_hat + B_d * u + L * error
     for (int i = 0; i < 4; i++) {
      x_hat_next[i] = 0.0;
...
/*
Serial.print("theta: ");
Serial.print(theta, 3);    // 3 decimales
Serial.print("\tphi: ");
Serial.print(phi, 3);
Serial.print("\tu: ");
Serial.print(u, 3);
Serial.print("\tu_servo: ");
Serial.println(u_servo, 3);*/
...
void matlab_send(float x1, float x2, float x3, float x4, float theta) {
  Serial.write("abcd");
  byte * b = (byte *)&x1;
  Serial.write(b, 4);
  b = (byte *)&x2;
